...
<div class="mb-3">
     ...
     <select name="author" class="form-control" required>
          <option value="" disabled {{ old('author', isset($post) ? $post->author : '') == '' ? 'selected' : '' }}>Pilih author....</option>
          @foreach ($user as $u)
               <option value="{{ $u->id }}" {{ old('author', isset($post) ? $post->author : '') == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
          @endforeach
     </select>
</div>
<div class="mb-3">
...
<div class="mb-3">
     ...
     <select name="kategori_id" class="form-control" required>
          ...
          @foreach ($kategori as $k)
               <option value="{{ $k->id }}" {{ old('kategori_id', isset($post) ? $post->kategori_id : '') == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
          @endforeach
     </select>
</div>
<div class="mb-3">
...
<div class="mt-3">
     ...
     <a href="/post" class="btn btn-danger"><i class="bi bi-arrow-left-circle"></i> Kembali</a>
</div>
